...
if "df_uploaded" in st.session_state:
    ...
    if kolom_analisis not in df.columns:
        st.warning(f"⚠️ Kolom {kolom_analisis} tidak tersedia di file yang diupload.")
        st.stop()
else:
    if basis_opsi == "Nama Produk":
        ...
        df = load_kategori()
    # Samakan nama kolom analisis jadi kolom_analisis
    df.rename(columns={kolom_analisis: "analisis_produk"}, inplace=True)
...
if page == "\U0001F4C3 Data Transaksi":
    ...
    # Gunakan kolom analisis produk (jika belum ada, buat alias dulu)
    if kolom_analisis in df_view.columns:
        df_view.rename(columns={kolom_analisis: "analisis_produk"}, inplace=True)
    elif "analisis_produk" not in df_view.columns:
        st.warning(f"⚠️ Kolom {kolom_analisis} tidak tersedia.")
        st.stop()
...
# Halaman Analisis
    ...
    # Cek ulang kolom analisis
    if kolom_analisis in df_fp.columns:
        df_fp.rename(columns={kolom_analisis: "analisis_produk"}, inplace=True)
    elif "analisis_produk" not in df_fp.columns:
        st.warning("⚠️ Kolom produk tidak ditemukan.")
        st.stop()
