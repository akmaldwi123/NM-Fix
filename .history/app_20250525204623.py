...
if page == "\U0001F4C3 Data Transaksi":
    ...
    df_view["tanggal"] = pd.to_datetime(df_view["tanggal"])  # pastikan timestamp
    ...
    tgl_start, tgl_end = st.date_input(
        ...
        min_value=min(tanggal_tersedia),
        max_value=max(tanggal_tersedia)
    )
    
    # Filter berdasarkan rentang tanggal
    df_view = df_view[
        (df_view["tanggal"] >= pd.Timestamp(tgl_start)) &
        (df_view["tanggal"] <= pd.Timestamp(tgl_end))
    ]
