...
if page == "📥 Upload File":
    ...
    if uploaded_file:
        try:
            ...
            elif not any(col in df_uploaded.columns for col in ["nama_produk", "kategori_produk"]):
                st.error("File harus memiliki salah satu kolom: nama_produk atau kategori_produk.")
            else:
                kolom_detect = "nama_produk" if "nama_produk" in df_uploaded.columns else "kategori_produk"
                df_filtered = df_uploaded[["no_transaksi", kolom_detect] + (["tanggal"] if "tanggal" in df_uploaded.columns else [])].copy()
                st.session_state.df_uploaded = df_filtered
                st.session_state.kolom_produk = kolom_detect
